...
void	create_map(t_game *game, int i)
{
	...
	while (game->map[i])
	{
		while (game->map[i][j])
		{
			...
			if (game->map[i][j] == 'E')
			{
				game -> exit_pos[0] = (j * SIZE);
				game -> exit_pos[1] = (i * SIZE);
			}
			image.path = block.path;
			put_image(game, image, (j * SIZE), (i * SIZE));
			j++;
		}
		i++;
	}
...
void	create_floor(t_game *game, int b)
{
	...
	image.game = *game;
	j = 0;
	if (ft_strlen(game -> map[b]) != game -> x / SIZE
		&& b != (game -> y / SIZE))
		message_destroy(RED"Error:\nThe map is not rectangular"RESET, 2, game);
	if (game -> map[(game -> y / SIZE)]
		&& game -> map[(game -> y / SIZE)][0] != '\0')
		message_destroy(RED"Error:\nInvalid map"RESET, 2, game);
	while (game -> map[b])
	{
		j = 0;
		while (game -> map[b][j])
		{
			if (!(ft_strchr("1PCE0", game -> map[b][j])))
				message_destroy(RED"Error:\nUknown element"RESET, 2, game);
			image.path = "./textures/floor.xpm";
			put_image(game, image, (j * SIZE), (b * SIZE));
			j++;
		}
		b++;
	}
...
void	create_map_line(t_game *game, char **av)
{
	...
	if (fd < 0)
		message_destroy(RED"Error:\nCouldn't open map!"RESET, 1, game);
	i = 0;
	while (i <= game -> y / SIZE)
	{
